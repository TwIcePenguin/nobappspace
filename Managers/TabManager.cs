...
namespace NOBApp.Managers
{
	...
	public class TabManager
	{
			...
			try
			{
				...
				foreach (TabItem tab in tabControl.Items)
				{
					if (tab.Content is NobMainCodePage page)
					{
						...
						{
							headerText = tab.Header?.ToString() ?? string.Empty;
						}
		...
		private void CreateTabItem(TabControl tabControl, TabItemState state, int index, List<NOBDATA> allNobWindows)
		{
			...
			// Fix for corrupted header text from previous save
			string displayText = state.Header;
			if (displayText == "System.Windows.Controls.StackPanel")
			{
				displayText = !string.IsNullOrEmpty(state.PlayerName) ? state.PlayerName : $"角色{index}";
			}

			// Create custom header with Button
			var headerPanel = new StackPanel { Orientation = Orientation.Horizontal };
			var headerText = new TextBlock { Text = displayText, VerticalAlignment = System.Windows.VerticalAlignment.Center };

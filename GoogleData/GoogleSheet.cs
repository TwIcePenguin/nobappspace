...
namespace NOBApp.GoogleData
{
    internal class GoogleSheet
    {
        ...
        static public async Task GoogleSheetInitAsync()
        {
        ...
        public static async Task CheckDonateAsync(NOBDATA nobData)
        {
            if (GoogleSheetValues != null)
            {
                var range = $"Get!A:D";
                ...
                if (values != null && values.Count > 0)
                {
                    ...
                    {
                        var foundValue = mappedValues.Find(v => v.FromName.Trim().Equals(nobData.Account.Trim(), StringComparison.OrdinalIgnoreCase));
                        if (foundValue != null)
                        {
                            ...
                            if (DateTime.TryParse(foundValue.EndTimer, out DateTime expirationDate) && foundValue.ISEND.Contains("NO"))
                            {
                                ...
                                await Authentication.儲存認證訊息Async(nobData, userData);
                            }
                        ...
                        {
                            await CheckTryTimeAsync(nobData);
                        }
                    }
                }
            }
        ...
        public static async Task CheckTryTimeAsync(NOBDATA nobData)
        {
            if (GoogleSheetValues != null)
            {
                var range = $"ACC!A:C";
                ...
                if (values != null && values.Count > 0)
                {
                    ...
                    if (mappedValues.Count > 0)
                    {
                        var foundValue = mappedValues.Find(v => v.Acc!.Trim().Equals(nobData.Account.Trim(), StringComparison.OrdinalIgnoreCase));
                        if (foundValue != null)
                        {
                            ...
                            await Authentication.儲存認證訊息Async(nobData, foundValue);
                        }
                    }
                }
            }
